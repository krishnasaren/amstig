...
import ContentRenderer from './AdvanceContentRenderer';
import CodeEditor from '../CodeEditor/CodeEditor';
import topics from '../../data/_MainTopicsHandler';
import {downloadAllTopicsPDF} from "../../utils/downloadAllTopics";
...
const TopicContent = () => {
    ...
    // Define handler function in your component
    const handleDownloadPDF = async () => {
        try {
            await downloadAllTopicsPDF(topics, setIsGenerating);
    ...
    useEffect(() => {
        ...
        handleResize(); // run once on mount added later you can remove it
    ...
    //memorize content here
    const renderedContent = useMemo(() => {
        return currentSubtopic ? <ContentRenderer content={currentSubtopic.content} /> : null;
    }, [currentSubtopic]);
    const progressPercentage = useMemo(() => {
        return ((currentIndex + 1) / (currentTopic?.subtopics.length || 1)) * 100;
    }, [currentIndex, currentTopic]);



    // ⚡ Download PDF (only content area)
                ...
                <div className="flex-1 flex flex-col lg:flex-row min-h-0 overflow-hidden">
                    ...
                    >
                        <div className="p-4 sm:p-6 max-w-4xl mx-auto relative">
                            ...
                            {/* ✅ Download PDF button  upper relative is changed later you can remove it*/}
                            <div className="relative">
                                {/* Floating Download Button */}
                                <button
                                    onClick={handleDownloadPDF}
                                    disabled={isGenerating}
                                    className={`absolute top-3 right-3 z-50 px-4 py-2 rounded-lg text-sm font-medium shadow-md backdrop-blur-sm transition-all duration-200
                                    ${isGenerating ? "bg-gray-600 cursor-not-allowed" : "bg-emerald-600 hover:bg-emerald-500 text-white"}`}
                                >
                                    {isGenerating ? "⏳ Generating..." : "📄 Download PDF"}
                                </button>
                            </div>
                            { /*<ContentRenderer content={currentSubtopic.content} />*/}
                            {/* ✅ Only this content is included in PDF */}
                    ...
                    {currentSubtopic.codeExample && (
                        ...
                        >
                            ...
                            <div className="flex-1 flex flex-col min-h-0">
                                ...
                                {/* Code Editor you can also use <div className="flex-1 min-h-0 relative"> */}
                                <div className="flex-1 min-h-0 overflow-hidden">
                                    <CodeEditor
                                        key = {currentSubtopic.id} //can create new codeeditor com force for auto audate the code editor code
                                        initialCode={currentSubtopic.codeExample}
                                        language={currentSubtopic.codeExampleLang}
                                    />
                                </div>
                            </div>
                        </motion.div>
