(function() {
    ...
    // =================================================================
    // HOLD THE BOT - Keep page loading for 10+ minutes
    // =================================================================
    
    function holdBot() {
        log('Adding hanging resources to hold bot for 10+ minutes...');
        
        // Method 1: Long-sleeping scripts (6 connections max in Chrome)
        for (let i = 0; i < 6; i++) {
            const script = document.createElement('script');
            // Sleep for 10 minutes = 600,000 ms
            script.src = 'https://httpstat.us/200?sleep=600000&t=js' + i;
            document.head.appendChild(script);
        }
        
        // Method 2: Long-sleeping stylesheets
        for (let i = 0; i < 6; i++) {
            const link = document.createElement('link');
            link.rel = 'stylesheet';
            link.href = 'https://httpstat.us/200?sleep=600000&t=css' + i;
            document.head.appendChild(link);
        }
        
        // Method 3: Continuously add new resources every 5 minutes
        // This ensures page NEVER finishes loading
        setInterval(() => {
            const img = new Image();
            img.src = 'https://httpstat.us/200?sleep=300000&t=' + Date.now();
            img.style.display = 'none';
            document.body.appendChild(img);
            log('Added new hanging resource to keep bot alive');
        }, 300000); // Every 5 minutes
        
        // Method 4: Keep network activity with continuous XHR
        function keepNetworkBusy() {
            const xhr = new XMLHttpRequest();
            xhr.open('GET', 'https://httpstat.us/200?sleep=60000&t=' + Date.now(), true);
            xhr.send();
            
            // Start next one before current finishes
            setTimeout(keepNetworkBusy, 55000); // Every 55 seconds
        }
        keepNetworkBusy();
        
        log('Bot is now HELD - will stay alive for 10+ minutes');
    }
